...
async function main() {
  console.log('🌱 Starting database seed for PSG5...');
  ...
  try {
    ...
    const admin1 = await prisma.user.findUnique({ where: { email: 'admin@psg.com' } });
    ...
    const jason = await prisma.user.findUnique({ where: { email: 'jason.walker@psg.local' } });
    ...
    console.log('Tables might already exist - this is OK if migrating to existing database');
  }
}
